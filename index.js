...
const logger = {
  info: (message) =>
    console.log(chalk.dim.blue("•") + chalk.dim(" info  - ") + message),
  ready: (message) =>
    console.log(chalk.dim.green("•") + chalk.dim(" ready - ") + message),
  warn: (message) =>
    console.log(chalk.dim.yellow("•") + chalk.dim(" warn  - ") + message),
  error: (message) =>
    console.log(chalk.dim.red("•") + chalk.dim(" error - ") + message),
  event: (message) =>
    console.log(chalk.dim.cyan("•") + chalk.dim(" event - ") + message),
};
      ...
      try {
        ...
        if (module && module.run && typeof module.run === "function") {
          ...
          if (module.params && module.params.length > 0) {
            fullPathWithParams +=
              "?" + module.params.map((param) => `${param}=`).join("&");
          }

          const category = module.category || "Other";
          const categoryIndex = endpoints.findIndex(
          ...
          const categoryObj = endpoints.find(
            (endpoint) => endpoint.name === category,
          );
          const endpointObj = {};
          ...
          logger.ready(
            `${chalk.green(endpointPath)} ${chalk.dim("(")}${chalk.cyan(category)}${chalk.dim(")")}`,
          );
        }
      } catch (error) {
...
const {
  ...
  getStats,
  setTotalEndpoints,
} = require("./lib/database/statistic");
...
logger.ready(
  `Loaded ${allEndpoints.reduce((total, category) => total + category.items.length, 0)} endpoints`,
);

app.get("/endpoints", (req, res) => {
  const totalEndpoints = allEndpoints.reduce(
